...
fun MultChoicesQuestionScreen(
    ...
    Column(
        ...
        currentQuestion.opciones.forEach { opcion ->
            ...
            val isDisabled = (isAcceptButtonClicked && immediateResults && !isCorrect) ||
                    (!immediateResults && quizTerminado && !isCorrect) // Deshabilitar respuestas incorrectas

            // El color del botón cambia solo si ya se hizo clic en el botón de aceptación
            val buttonColor = when {
                (isAcceptButtonClicked && immediateResults && isCorrect) || (!immediateResults && quizTerminado && isCorrect)  -> Color.Green // Verde para respuestas correctas cuando se ha aceptado
